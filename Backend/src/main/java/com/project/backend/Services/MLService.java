...
public class MLService {
    ...
    public Optional<String> predictAlgorithm(String inputHex) {
        ...
        try {
            ...
            // Don't redirect stderr to stdout - we only want the prediction result
            Process process = processBuilder.start();

            // Read output from Python script (stdout only)
            BufferedReader reader = new BufferedReader(new InputStreamReader(process.getInputStream()));
            StringBuilder output = new StringBuilder();
            String line;
            while ((line = reader.readLine()) != null) {
            ...
            log.info("Returning the predicted algorithm: " + result);
